        ...
        <div class="main">
            <div class="article-content">
                ...
                <div class="article-actions">
                    <button
                        class="article-actions-item likes"
                        :class="{ clicked: isLike == 'true' }"
                        @click="ArticleLike">
                        {{ isLike == "true" ? "已点赞" : "点赞" }}
                    </button>
                    <button
                        class="article-actions-item collection"
                        :class="{ clicked: isCollection == 'true' }"
                        @click="ArticleCollection">
                        {{ isCollection == "true" ? "已收藏" : "收藏" }}
                    </button>
                </div>
                            ...
                            <div class="comment-item" v-for="comment in comments" :key="comment">
                                ...
                                <div class="comment-item-content">
                                    ...
                                    <div class="comment-actions">
                                        ...
                                        <div class="action-item comm" @click="actionItemComment(comment.id, 0)">
                                            评论
                                        </div>
                                    ...
                                    <div class="child-comment-input" :id="'child-comment-' + comment.id">
                                        ...
                                        <button
                                            @click="
                                                ReleaseComment(comment.id, comment.user.id, comment.user.username, 1)
                                            ">
                                            评论
                                        </button>
                                    </div>
                                        ...
                                        <div class="child-comments-items" :id="'comment-' + comment.id">
                                            <div
                                                ...
                                                <div class="comment-item-content">
                                                    ...
                                                    <div
                                                        class="child-comment-input"
                                                        :id="'child-comment-' + chComment.id">
                                                        <input
                                                            type="text"
                                                            class="child-comment-textarea"
                                                            placeholder="回复楼主"
...
async function getComment() {
    await getArticleComments(currPage.value, pageSize.value, sortValue.value, route.params.id).then((res) => {
...
function downloadAttachment(file) {
    ...
    // // link.setAttribute("download", file.fileOriginName);
...
async function ReleaseComment(parentId = 0, toUserId = 0, toUsername = null, flag = 0) {
    var articleId = parseInt(route.params.id);
    let content;
    if (flag == 0) {
...
async function getUserLikeComm(comm, f) {
    //TODO if (localStorage.getItem(md5("token")) != md5("nologin")) {
    if (localStorage.getItem(md5("token")) != null) {
        await getUserLikeComment(route.params.id).then((res) => {
            if (res.data.code == 1) {
...
function likeComm(comm) {
    ...
    //         offset: 120,
    //     });
    //     clearTimeout(csleepLikeTimer);
    //     csleepLikeTimer = setTimeout(() => {
    //         csleeplike = 0;
    //     }, 3000);
    //     return;
    // }

    // let c = Object.assign({}, comm);
    // if (isCommLiked(c)) {
    //     comm.likeCount--;
    //     userLikeComm.value.filter((i) => {
    //         i == c.id;
    //     });

    //     c.likeCount = 1;
    // } else {
    //     userLikeComm.value.push(c.id);
    //     comm.likeCount++;
    //     c.likeCount = 0;
    // }
...
function ArticleLike() {
    ...
    isLike.value == "true" ? ((likeflag = 1), (isLike.value = "false")) : ((likeflag = 0), (isLike.value = "true"));
...
function IsArticleLike() {
    //TODO if (localStorage.getItem(md5("token")) != md5("nologin")) {
    if (localStorage.getItem(md5("token")) != null) {
        isArticleLike({ articleId: route.params.id }).then((res) => {
            if (res.data.code == 1) {
                isLike.value = res.data.msg;
                isCollection.value = res.data.data;
            }
        });
    }
}
...
function setHeaderBg() {
    ...
    header.style.backgroundImage = "url(" + article.value.cover + ")";
}
/* 根据加载进度，调整模糊 */
let loadValue = 0;
function loadBlur(a) {
