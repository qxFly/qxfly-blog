    ...
    <div class="main">
      <div class="article-content">
        ...
        <div
          ...
          @click="searchClassifyTag(article.classify, 0)">
      ...
      <div class="sider-right">
        <div class="sider-right-content" id="sider-right-content">
          ...
          <CardView v-if="showAttachmens(0)">
              ...
              <div
                ...
                <div class="input-attachment-list-item-content">
                  <div style="display: flex; width: 100%">
                    ...
                    <div class="input-attachment-list-item-name">
                      {{ file.fileOriginName }}
                    </div>
                  </div>
                  <div
                  ...
                  <div
                    ...
                    :id="
                      'input-attachment-process-number-' + file.fileName
                    "></div>
                </div>
            ...
            <div v-if="!showAttachmens(1)">
              <a href="/login" style="color: red">登录</a> 后查看附件
            </div>
          </CardView>
          ...
          <div
            ...
            v-if="
              ...
              mydm5.generateMD5('true' + route.params.salt)
            ">
            <div class="sider-edit-Btn" @click="EditArticle">
              编&emsp;&emsp;&emsp;辑
            </div>
            <div class="sider-edit-Btn" @click="DeleteArticle">
              删&emsp;&emsp;&emsp;除
            </div>
          </div>
        </div>
...
async function getArticleDetail() {
  ...
  await getArticleById(route.params.id).then((res) => {
    ...
    if (article.value.updateTime != null)
      article.value.updateTime = new Date(
        article.value.updateTime,
      ).toLocaleString();
    if (article.value.createTime != null)
      article.value.createTime = new Date(
    ...
    for (let i = 0; i < imgEles.length; i++) {
      ...
      imgsrc =
        process.env.VUE_APP_ARTICLE_IMG_PATH + "articleImage/" + split[1];
...
function downloadAttachment(index, file) {
  ...
  let cancelDownloadEle = document.getElementById(
    "input-attachment-cancelDownload-" + index,
  );
...
function cancelDownload(index, file) {
  downloadControllerList.value
    .find((item) => item.index == index)
    .controller.abort();
}
/* 下载进度条 */
function onProcess(e, file, index) {
  const processEle = document.getElementById(
    "input-attachment-process-" + file.fileName,
  );
  const processnumber = document.getElementById(
    "input-attachment-process-number-" + file.fileName,
  );
...
function ArticleCollection() {
  ...
  articleCollection({ aid: route.params.id, flag: collectionflag }).then(
    (res) => {
      if (res.data.code != 1) {
        ElMessage({
          message: res.data.msg,
