      ...
      <div class="progress">
        ...
        <span v-if="(uploadFile.uploadPercentage > 0) & (uploadFile.uploadPercentage < 100)">
          ...
          <el-button circle link @click="changeUploadingStop(uploadFile)">
            <el-icon size="20" v-if="uploadFile.uploadingStop == false"><VideoPause /></el-icon>
...
function upload(xhrData) {
  ...
  for (var i = 0; i < uploadFileList.value.length; i++) {
    if ((xhrData.file.name == uploadFileList.value[i].name) & (xhrData.file.size == uploadFileList.value[i].size)) {
      ...
      if (uploadFile.chunkList != null) {
        console.log("uploadFile.chunkList:" + uploadFile.chunkList);
        uploadFile.chunkList = [];
        console.log("uploadFile.chunkList:" + uploadFile.chunkList);
      }
      // console.log("uploadFileList:" + JSON.stringify(uploadFileList));
      break;
    }
  }
